<?php
  ...
  //      $id = $_POST['id'];
    	...
    	if (empty($id)){
            ...
            $sql .= "nome, cpf, fone, celular, iswhats, datanasc, genero, endereco, bairro, cidade, ";
            $sql .= "uf, cep, email, senha, adm) values ";
            $sql .= "('$nome', '$cpf', '$fone', '$celular', '$iswhats', '$datanasc', '$genero', ";
            $sql .= "'$endereco', '$bairro', '$cidade', '$uf', '$cep', '$email', '$senha', '$adm')";
        }else{
...
?>
        ...
        <header>
            <div id="logo"><img src="img/logob.png"></div><h2>Cadastro de Usuários</h2>
          </header>
          
          <section>
            <nav style=height:1450px>
                <?php
